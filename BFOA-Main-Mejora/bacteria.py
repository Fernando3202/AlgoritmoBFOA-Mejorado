...
class bacteria():
    ...
    def __init__(self, path):
        ...
        self.interaction = 0
        self.NFE = 0
        self.previous_score = float('-inf')  # Para tracking de mejoras
    ...
    def autoEvalua(self):   
        ...
        for i in range(len(self.matrix.seqs[0])):
            ...
            # Cuenta y elimina gaps de la columna
            gapCount = column.count("-")
            column = [x for x in column if x != "-"]
            
            # Recorre todos los pares únicos y calcula el score
            pares = self.obtener_pares_unicos(column)
            for par in pares:
                score += evaluador.getScore(par[0], par[1])
            
            # Penalización por gaps, ajustada para ser menos agresiva
            score -= gapCount * (1 / len(self.matrix.seqs))
        
        # Asegurarse de que el score sea positivo
        self.blosumScore = max(score, 0)
        self.NFE += 1
